    ...
    <div className="fixed inset-0 z-50 bg-background/95 backdrop-blur-xl flex flex-col md:flex-row animate-in fade-in duration-500">
      ...
      <div className="w-full md:w-[70%] h-[60vh] md:h-full relative">
        ...
        {step === 1 && (
          <div className="absolute inset-0 flex items-center justify-center p-4">
            <div className="border border-white/20 border-dashed rounded-lg p-10 md:p-16 text-center bg-background/80 backdrop-blur-xl shadow-[0_0_80px_rgba(0,0,0,0.8)] max-w-lg w-full transition-colors hover:border-primary/40">
      ...
      <div className="w-full md:w-[30%] h-[40vh] md:h-full bg-[#080808] flex flex-col p-8 md:p-12 justify-between overflow-y-auto">
            ...
            <div className="flex flex-col gap-10 animate-in slide-in-from-right-4 duration-700">
              ...
              <div className="flex flex-col gap-4">
                <div className="flex justify-between">
                  ...
                  <span className="text-[10px] text-primary font-sans">{skewX}°</span>
                </div>
                <input type="range" min="-45" max="45" step="1" value={skewX} onChange={(e) => setSkewX(parseInt(e.target.value))} className="w-full accent-primary bg-white/10 h-[2px] appearance-none cursor-pointer" />
              </div>

              <div className="flex flex-col gap-4">
                <div className="flex justify-between">
                  <label className="text-[10px] text-muted uppercase tracking-[0.2em] font-sans">Visual Scale Output</label>
                  <span className="text-[10px] text-primary font-sans">{(scale * 100).toFixed(0)}%</span>
                </div>
                <input type="range" min="0.1" max="2.0" step="0.05" value={scale} onChange={(e) => setScale(parseFloat(e.target.value))} className="w-full accent-primary bg-white/10 h-[2px] appearance-none cursor-pointer" />
              </div>
        ...
        <div className="flex items-center justify-center gap-3 mt-auto pt-10">
          {[1, 2, 3, 4].map((s) => (
            <div key={s} className={`w-2 h-2 rounded-full transition-colors duration-[600ms] ease-editorial ${step === s ? 'bg-primary' : 'bg-outline-variant/30'}`} />
          ))}
        </div>

        {step === 4 && (
          <div className="pt-8 animate-in slide-in-from-bottom-4 duration-700 delay-300">
            <Button onClick={triggerExport} className="w-full py-6 uppercase tracking-[0.2em] text-[11px] bg-primary text-[#3c2f00] hover:brightness-110 shadow-ambient transition-all duration-[600ms] ease-editorial font-bold h-14">
              Commence Render
            </Button>
          </div>
        )}
